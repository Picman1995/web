...
import { serviceCreateTipoVinculos } from '../../../services/tipoVinculos/tipoVinculos'; // Ajusta la importación del servicio de crear país
import '../../../components/EstandarCSS/Insert.css';
export const AddTipoVinculos = () => {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await serviceCreateTipoVinculos({ descripcion }); // Utiliza el servicio de crear país
      setShowConfirmation(true);
      setTimeout(() => {
        setShowConfirmation(false);
        navigate('/tipoVinculos');
    ...
    {showConfirmation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-custom-naranja p-8 rounded-xl shadow-lg text-center max-w-md">
            ...
            <h2 className="text-2xl font-bold text-white mb-4">Tipo Vinculos Agregado!</h2>
            <p className="text-white text-lg mb-4">Se ha insertado un nuevo tipo Vinculos.</p>
            <button
      ...
      <div className='text-custom-black p-12 bg-gray-100 sm:bg-white sm:rounded-xl flex flex-col sm:justify-center sm:align-baseline gap-14 absolute top-0 left-0 bottom-0 right-0 sm:static w-full h-auto sm:w-auto sm:h-auto overflow-auto sm:overflow-visible'>
        <h3 className='titulo_general_A '>
          Agregar Tipo Vinculos
        </h3>
        <div className='flex sm:hidden mb-0 justify-normal items-center'>
          <h3 className='text-xl  items-start ml-2'>Agregar Tipo Vinculos</h3>
        ...
        <div className='w-full grid grid-cols-1 sm:grid-cols-2 gap-7 '>
          <div className='flex flex-col'>
            ...
            <input
              ...
              placeholder='Ingresa un tipo Vinculos'
              required
            />
          </div>
        </div>
